...
On every input event, makima writes a fully-resolved state snapshot to `$XDG_RUNTIME_DIR/makima-state.json` for the HUD. The trackpads are exposed as standard uinput MT devices for libinput and gesture tools, and combined into a third virtual multi-touch device for two-finger gesture recognition.
