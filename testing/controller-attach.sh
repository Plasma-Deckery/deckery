#!/bin/bash
# Pass the Steam Deck Controller (28de:1205) through to the Bazzite test VM.
# The host loses access while the VM holds the device.
# Use controller-detach.sh to return it to the host.
#
...
USB_INFO=$(echo "info usb" | socat - "$SOCK" 2>/dev/null)
if echo "$USB_INFO" | grep -q "steamctrl"; then
  echo "✓ Controller → VM (host no longer has access)"
else
